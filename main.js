...
class Particle {
  constructor(x, y, delay, speed, image, spline) {
    this.x = x;
    this.y = y;
    this.image = image;
    this.t = 0;
...
class ParticleManager {
  ...
  addParticle(x, y, delay, points, src, speed, spline) {
    const img = new Image();
    img.src = src;
    const particle = new Particle(x, y, delay, speed, img, spline);
    particle.precomputed = catmullRomPrecompute(points);
    this.particles.push(particle);
  }

  update() {
    this.particles.forEach((p) => p.update(p.precomputed));
  }
